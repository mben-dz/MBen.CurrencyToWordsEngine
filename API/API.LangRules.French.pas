...
function TFrenchRules.GetLangCode: string; begin Result := 'fr'; end;
...
function TFrenchRules.FormatHundred(aHundred: Integer; isPlural: Boolean): string;
begin
  if aHundred = 1 then
    Result := 'cent' else
    if isPlural then
      Result := Unit_(aHundred) + ' cents' else
      Result := Unit_(aHundred) + ' cent';
end;
...
begin
  if aCount > 1 then Result := cScalesPlural[aScale] else Result := cScalesSingular[aScale];
end;

function TFrenchRules.CombineTens(const aTens, aUnits: string; aNumber: Integer): string;
begin
  ...
  else if (aNumber mod 10 = 1) and (aNumber < 70) then Result := aTens + ' et un'
...
function TFrenchRules.CombineHundreds(const aHundred, aRest: string): string;
begin
