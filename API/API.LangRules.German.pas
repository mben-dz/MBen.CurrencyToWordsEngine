...
function TGermanRules.GetLangCode: string; begin Result := 'de'; end;
...
begin
  Result := T[aTen div 10];
end;

function TGermanRules.FormatHundred(aHundred: Integer; isPlural: Boolean): string;
begin
  if aHundred = 1 then
    Result:='einhundert' else
    Result:=Unit_(aHundred)+'hundert';
end;

function TGermanRules.ScaleWord(aScale: TScale; aCount: Int64): string;
const S: array[TScale] of string = (
  'tausend','Million','Milliarde','Billion','Billiarde','Trillion');

const P: array[TScale] of string = (
  'tausend','Millionen','Milliarden','Billionen','Billiarden','Trillionen');
begin
  if aCount>1 then
    Result:=P[aScale] else
    Result:=S[aScale];
end;

function TGermanRules.CombineTens(const aTens, aUnits: string; aNumber: Integer): string;
begin
  if aUnits='' then
    Result := aTens else
    if (aNumber mod 10) = 1 then
      Result := 'einund'+aTens else
      Result := aUnits+' und '+aTens;
end;
...
function TGermanRules.FormatScale(aCount: Int64; const aCountWord, aScaleWord: string): string;
begin
  if (aScaleWord = 'tausend') and (aCount = 1) then
    Result := 'eintausend' else
    if aScaleWord = 'tausend' then
      Result := aCountWord +aScaleWord else
      if aCount = 1 then
        Result := 'eine ' +aScaleWord else
        Result := aCountWord+' '+aScaleWord;
end;

function TGermanRules.CombineScales(const aScaleWord, aRest: string): string;
begin
  if aScaleWord.EndsWith('tausend') then
    Result := aScaleWord +aRest else
