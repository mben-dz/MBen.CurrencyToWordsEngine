...
begin
  if aNumber < 0 then Exit(aLangRule.NegativePrefix + SpellInteger(-aNumber, aLangRule));
  ...
  begin
    ...
    begin
      if LResult <> '' then
        LResult := aLangRule.CombineScales(
          aLangRule.FormatScale(LGroup,
                                SpellInteger(LGroup, aLangRule),
                                aLangRule.ScaleWord(LScale, LGroup)),
          LResult)
      else
        LResult := aLangRule.FormatScale(LGroup,
                                         SpellInteger(LGroup, aLangRule),
                                         aLangRule.ScaleWord(LScale, LGroup));
    end;
  end;
